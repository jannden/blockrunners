...
describe("Purchase ciphers", () => {
  ...
  it("Allows player to purchase ciphers", async () => {
    ...
    const expectedPrizePoolIncrease = Math.floor((expectedCost * PRIZE_POOL_PERCENTAGE) / 100);
    const expectedAdminShare = expectedCost - expectedPrizePoolIncrease;
    ...
    expect(gameStateAfter.prizePool.toNumber()).to.equal(
      gameStateBefore.prizePool.toNumber() + expectedPrizePoolIncrease
    );

    // Verify admin received their share
    expect(adminBalanceAfter - adminBalanceBefore).to.equal(expectedAdminShare);

    // Verify game balance increased by only the prize pool portion
    expect(gameBalanceAfter - gameBalanceBefore).to.equal(expectedPrizePoolIncrease);
